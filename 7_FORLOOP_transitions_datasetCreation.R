...
setwd('D:/PreLab/NewZealand/double_step_model/Axivity_data/lab/6_transition_noTransition')


# Load the data
data <- read.csv(file = 't_nT_lab_data.csv', header = TRUE)
...
filtered_data <- data.frame(b_x_noise = signal::filtfilt(f_noise, data$x_b),
...
filtered_data2 <- data.frame(b_x_g = signal::filtfilt(f_g, filtered_data$b_x_noise),	
...
filtered_data3 <- data.frame(t_x_noise = signal::filtfilt(f_noise, data$x_t),
...
filtered_data4 <- data.frame(t_x_g = signal::filtfilt(f_g, filtered_data3$t_x_noise),	
...
# Gather original data and filtered data
data <- cbind(data, filtered_data, filtered_data2, filtered_data3, filtered_data4)
...
for (i in ((window_width/2) + 1):(nrow(data)-((window_width/2) + 1))) {
#for (i in ((window_width/2) + 1):360000) {
  ...
  df <- data[(i-(window_width/2)):(i+(window_width/2)),]
    
    # information
    X = df$X[(window_width/2) + 1];
    subject = df$subject[1];
    video_time = df$video_time[(window_width/2) + 1];
    axiv_index_b = df$axiv_index_b[(window_width/2) + 1];
    axiv_index_t = df$axiv_index_t[(window_width/2) + 1];
    lim_activ_0_2 = df$lim_activ_0_2[(window_width/2) + 1];
    ...
    put(X, subject, video_time, axiv_index_b, axiv_index_t, lim_activ_0_2, b_count, t_count, b_mean_x, b_mean_y, b_mean_z, t_mean_x, t_mean_y,
         t_mean_z, b_mag_mean, t_mag_mean, b_sd_x, b_sd_y, b_sd_z, t_sd_x, t_sd_y, t_sd_z,
         b_mag_sd, t_mag_sd, b_coef_x, b_coef_y, b_coef_z, b_mag_coef, t_coef_x, t_coef_y, 
         t_coef_z, t_mag_coef, b_quan_25_x, b_quan_25_y, b_quan_25_z, t_quan_25_x, t_quan_25_y,
         t_quan_25_z, b_mag_quan_25, t_mag_quan_25, b_quan_50_x, b_quan_50_y, b_quan_50_z,
         t_quan_50_x, t_quan_50_y, t_quan_50_z, b_mag_quan_50, t_mag_quan_50, b_quan_75_x,
         b_quan_75_y, b_quan_75_z, t_quan_75_x, t_quan_75_y, t_quan_75_z, b_mag_quan_75,
         t_mag_quan_75, b_min_x, b_min_y, b_min_z, t_min_x, t_min_y, t_min_z, b_mag_min, 
         t_mag_min, b_max_x, b_max_y, b_max_z, t_max_x, t_max_y, t_max_z, b_mag_max, 
         t_mag_max, corr_xTyT, corr_xTzT, corr_yTzT, corr_xByB, corr_xBzB, corr_yBzB,
         corr_xTxB, corr_xTyB, corr_xTzB, corr_yTxB, corr_yTyB, corr_yTzB, corr_zTxB,
         corr_zTyB, corr_zTzB, corr_mTmB, mean_xBxT, mean_yByT, mean_zBzT, mean_xByT,
         mean_xBzT, mean_yBzT, mean_yBxT, mean_zBxT, mean_zByT, t_roll_mean, b_roll_mean,
         t_pitch_mean, b_pitch_mean, t_yaw_mean, b_yaw_mean, t_roll_sd, b_roll_sd, 
         t_pitch_sd, b_pitch_sd, t_yaw_sd, b_yaw_sd, t_mean_g_x, t_mean_g_y, t_mean_g_z,
         b_mean_g_x, b_mean_g_y, b_mean_g_z, t_mag_g_mean, b_mag_g_mean, t_fft_freq_x,
         t_fft_freq_y, t_fft_freq_z, t_fft_pow_x, t_fft_pow_y, t_fft_pow_z, t_fft_mag_freq,
         t_fft_mag_pow, b_fft_freq_x, b_fft_freq_y, b_fft_freq_z, b_fft_pow_x, b_fft_pow_y,
         b_fft_pow_z, b_fft_mag_freq, b_fft_mag_pow, t_peaks, b_peaks, t_kurtosis_x,
         t_kurtosis_y, t_kurtosis_z, b_kurtosis_x, b_kurtosis_y, b_kurtosis_z, t_skew_x,
         t_skew_y, t_skew_z, b_skew_x, b_skew_y, b_skew_z) 
...
# Remove the beginning and end of each subject
final_features <- temporary_features %>% dplyr::filter(subject == 'C001')
final_features <- final_features[1:(nrow(final_features)-((window_width/2) + 1)),]

for (i in unique (temporary_features %>% dplyr::filter(subject != 'C001') %>% dplyr::filter(subject != 'C043'))) {
...
subject_features <- temporary_features %>% dplyr::filter(subject == 'C043')
final_features <- rbind(final_features, subject_features[((window_width/2) + 1):nrow(subject_features),])


# Create a new file
setwd('D:/PreLab/NewZealand/double_step_model/Axivity_data/lab/7_transitions_datasetCreation')

write.csv(x = data, file = 'lab_data_transitions_0_2.csv', row.names = TRUE)